...
shared/{ssgl}/:: Any non-component files that you want to be able to use when live rendering can be placed here. A page helper has been created, which helps render arrays of components.
...
`@bookshop/init` created a `sample.scss` file for you in the `sample` component. This file will currently be empty, but you should add a style here to test that the Bookshop SCSS integration is working for you.
...
Your blueprint represents the initial state of your component. This can hinder your experience with the component browser, where you want to see a preview of your component in a real-world use-case.
...
IMPORTANT: Object Structures in CloudCannon will be initialized empty, so testing for the existence of this component in your template is recommended.
...
Any page that contains the `bookshop_component_browser` snippet will get picked up by `npx @bookshop/generate` and turned into a hosted component browser, no extra configuration is needed.
...
Once you have components rendered on the page, Bookshop will assign link:https://cloudcannon.com/documentation/articles/what-are-visual-data-bindings/?ssg={ssg}[Visual Data Bindings] automatically. If a component is passed data from the page front matter, you will be able to interact with that component directly on the page. 

By default, Bookshop will add bindings for any components on the page, but will not add bindings for shared includes/partials. This prevents Bookshop rendering data bindings around our shared `page` helper, so that the components within are immediately accessible.

The behavior of each scenario can be customised by including a flag in the component's data. Bookshop will look for any of the following keys:
...
For data to be accessible, you will need to set `data_config: true` in your `cloudcannon.config.*` file.
...
For data to be accessible, you will need to expose each data set using `data_config` in your `cloudcannon.config.*` file.
...
In order to work when live editing, Bookshop needs a clear path between a Bookshop component and the data it draws from. In general, you should avoid adding logic around your Bookshop components in your site layouts, and instead move that logic into a Bookshop component or helper.
...
```html
<!-- 
  This component can make the connection between "text" and ".Params.title",
  and will work as expected in the visual editor.
-->
{{ partial "bookshop_bindings" `(dict text .Params.title)` }}
{{ partial "bookshop" (slice "hero" (dict "text" .Params.title)) }}
...
<!-- 
  ...
  and will work as expected in the visual editor.
-->
{% bookshop hero text=page.title %}

{% assign my_title = title %}
...
=== Disabling Live Rendering

Bookshop can disable live rendering based on a flag in the component's data. Bookshop will look for any of the following keys:
...
If you have a specific component that you never want to live render, you can set `_live_render` in the component's blueprint.

NOTE: Since the blueprint only affects the creation on new components, you will need to add the `_live_render` flag to any existing component data in your front matter.
