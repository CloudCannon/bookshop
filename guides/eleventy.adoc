...
Within your Vite config, specify a `$components` alias with the path to your Bookshop project. 
...
const config = {
    ...
    resolve: {
        alias: {
            $components: resolve('./component-library/')
        }
    },
    server: {
        fs: {
...
TIP: Make sure that `$components` points to the component library you just created, relative to your SvelteKit source.
...
ifdef::sveltekit[]
Bookshop {ssg} does not yet support embedding the Bookshop browser in a website.
endif::sveltekit[]
